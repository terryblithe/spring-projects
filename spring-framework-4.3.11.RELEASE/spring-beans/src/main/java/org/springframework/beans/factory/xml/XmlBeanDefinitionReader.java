...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	 ...
	 * if none specified.
	 */
	// EntityResolver：用于 SAX 实现自定义处理外部实体，需要实现该接口，并使用 setEntityResolver 方法向 SAX 驱动器注册一个实例。
	// EntityResolver 的作用由程序来实现寻找 DTD 声明的过程
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		// EncodedResource 对资源文件进行封装
		// 作用：对资源文件的编码进行处理；当设置了编码属性的时候，Spring 会使用相应的编码作为输入流的编码
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		// 通过该属性(currentResources)来记录已经加载的资源
		...
		try {
			// 从 encodedResource 中获取已封装的 Resource 对象，并再次从 Resource 中获取其中的 inputStream
			InputStream inputStream = encodedResource.getResource().getInputStream();
			try {
				// InputSource 这个类并不来自 Spring，它的全路径是：org.xml.sax.InputSource
                // 准备 InputSource 对象，用于 SAX 读取 XML 文件
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
			throws BeanDefinitionStoreException {
		try {
            // 1. 获取对 XML 文件的验证模式，DTD | XSD
            // 2. 加载 XML 文件，并得到对应的 Document 实例
			Document doc = doLoadDocument(inputSource, resource);
            // 3. 根据 Document 注册 Bean 信息
	...
	protected int detectValidationMode(Resource resource) {
		...
		try {
            // 委托给 XmlValidationModeDetector 的 detectValidationMode() 方法检测验证模式
